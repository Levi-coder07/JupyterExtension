...
After editing Python files (like `LinkMaker/routes.py`):

```bash
python -m py_compile LinkMaker/__init__.py  # Check single file for syntax errors
```
...
- Backend processing in `LinkMaker/routes.py`
...
- **✅ Do**: `LinkMaker/` with underscores, all lowercase
...
export namespace CommandIDs {
  export const openPanel = 'LinkMaker:open-panel';
  export const refreshData = 'LinkMaker:refresh-data';
}
...
- Backend routes: `LinkMaker/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `LinkMaker/` directory
...
1. **Read the backend first** — Check `LinkMaker/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `LinkMaker/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "LinkMaker", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `LinkMaker/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `LinkMaker/`)
...
5. **Install the extension** (REQUIRED for JupyterLab to recognize it):
   ...
   jupyter server extension enable LinkMaker
   ```
...
- **Output**: Creates files in `lib/` and `LinkMaker/labextension/`
...
**After editing Python** (files in `LinkMaker/`):
...
- Ask user to search for the extension ID (`LinkMaker`) to see if it loaded
...
ls -la LinkMaker/labextension/  # Should contain bundled extension
```
...
- Backend routes: `LinkMaker/routes.py`
- Server extension config: `jupyter-config/server-config/LinkMaker.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `LinkMaker/routes.py`
...
**❌ Don't**: Use absolute imports like `from LinkMaker.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'LinkMaker:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'LinkMaker:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-LinkMaker-ClassName`
- **API routes** (in `LinkMaker/routes.py`): `url_path_join(base_url, "LinkMaker", "endpoint")`
